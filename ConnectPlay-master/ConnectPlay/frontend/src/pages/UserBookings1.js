...
import { getAllBookings1 } from "../redux/actions/bookingActions";
import { Col, Row } from "antd";
import Spinner from '../components/Spinner';
import moment from "moment";
function UserBookings1() {
  const dispatch = useDispatch();
  const { bookings } = useSelector((state) => state.bookingsReducer1);
  const {loading} = useSelector((state) => state.alertsReducer1);
  const user = JSON.parse(localStorage.getItem("user"));
  useEffect(() => {
    dispatch(getAllBookings1());
  }, []);
        ...
        <Col lg={16} sm={24}>
            ...
            {bookings.filter(o=>o.user==user._id).map((booking) => {
             return <Row gutter={16} className="bs1 mt-3 text-left">
                <Col lg={6} sm={24}>
                    <p><b>{booking.game.name}</b></p>
                    <p>Total hours : <b>{booking.totalHours}</b></p>
                    <p>Rent per hour : <b>{booking.game.rentPerHour}</b></p>
                ...
                <Col lg={6} sm={24} className='text-right'>
                    <img style={{borderRadius:5}} src={booking.game.image}  height="140" className="p-2"/>
                </Col>
              </Row>;
            })}
